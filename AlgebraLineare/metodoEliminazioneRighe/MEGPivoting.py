...
def MEG_Pivoting(A, b):
    ...
    for j in range(0, n - 1):
        ...
        amax = abs(A[j, j]) # Inizializzo il valore massimo con il primo elemento della diagonale
        imax = j # Indice dell'elemento massimo
...
A = np.array([[3.0, 2, 1, 4], [4, 2, 6, 1], [-5, 3, 2, 1], [3, 7, 1, 4]])
n = len(A)

xs = np.ones((n, 1)) # Vettore soluzione di tutti 1
b = np.dot(A, xs) # Vettore dei termini noti

[U, c] = MEG_Pivoting(A, b) # Applico l'algoritmo di eliminazione delle righe di Gauss su A, per ottenere C
